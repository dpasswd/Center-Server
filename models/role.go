...
func (role *SysRole) GetPage(pageSize int, pageNo int) ([]SysRole, int, error) {
	var doc []SysRole

	table := orm.Eloquent.Select("*").Table("dhp_role")
	if role.RoleId != 0 {
	...
	table, err := dataPermission.GetDataScope("dhp_role", table)
	if err != nil {
		return nil, 0, err
	}
	var count int

	if err := table.Order("role_sort").Offset((pageNo - 1) * pageSize).Limit(pageSize).Find(&doc).Error; err != nil {
		return nil, 0, err
	}
...
func (role *SysRole) GetFindPage(pageSize int, pageNo int, find string) ([]SysRole, int, error) {
	var doc []SysRole
	...
	if find != "" {
		table = table.Where("role_name like ? or role_key like ? or role_id like ?", "%"+find+"%",
			"%"+find+"%", "%"+find+"%")
	}
...
func (role *SysRole) Get() (SysRole SysRole, err error) {
	table := orm.Eloquent.Table("dhp_role")
	if role.RoleId != 0 {
		table = table.Where("role_id = ?", role.RoleId)
	}
	if role.RoleName != "" {
...
func (role *SysRole) GetRoleMeunId() ([]int, error) {
	...
	if err := orm.Eloquent.Table("dhp_role_menu").Select("dhp_role_menu.menu_id").Where("role_id = ? ", role.RoleId).Where(" dhp_role_menu.menu_id not in(select dhp_menu.parent_id from dhp_role_menu LEFT JOIN dhp_menu on dhp_menu.menu_id=dhp_role_menu.menu_id where role_id =?  and parent_id is not null)", role.RoleId).Find(&menuList).Error; err != nil {
		return nil, err
	}
...
func (role *SysRole) GetRoleDeptId() ([]int, error) {
	...
	if err := orm.Eloquent.Table("dhp_role_dept").Select("dhp_role_dept.dept_id").Joins("LEFT JOIN sys_dept on sys_dept.dept_id=dhp_role_dept.dept_id").Where("role_id = ? ", role.RoleId).Where(" dhp_role_dept.dept_id not in(select sys_dept.parent_id from dhp_role_dept LEFT JOIN sys_dept on sys_dept.dept_id=dhp_role_dept.dept_id where role_id =? )", role.RoleId).Find(&deptList).Error; err != nil {
		return nil, err
	}
