...
func (e *SysDept) GetPageList(pageSize int, pageNo int) ([]SysDept, int, error) {
	var doc []SysDept
...
func (e *SysDept) GetFindPageList(pageSize int, pageNo int, find string) ([]SysDept, int, error) {
	var doc []SysDept
	...
	if find != "" {
		table = table.Where("dept_name like ? or dept_id like ? or leader like ?", "%"+find+"%",
			"%"+find+"%", "%"+find+"%")
	}
