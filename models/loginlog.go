...
func (e *LoginLog) GetPage(pageSize int, pageNo int) ([]LoginLog, int, error) {
	var doc []LoginLog
	...
	if err := table.Order("info_id desc").Offset((pageNo - 1) * pageSize).Limit(pageSize).Find(&doc).Error; err != nil {
		return nil, 0, err
	}
