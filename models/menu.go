...
func (e *Menu) GetByRoleName(rolename string) (Menus []Menu, err error) {
	table := orm.Eloquent.Table(e.TableName()).Select("dhp_role.*").Joins("left join dhp_role_menu on dhp_role_menu.menu_id=dhp_role.menu_id")
	table = table.Where("dhp_role_menu.role_name=? and menu_type in ('M','C')", rolename)
	if err = table.Order("sort").Find(&Menus).Error; err != nil {
		return
	}
	return
...
func InitPaths(menu *Menu) (err error) {
	...
	if int(menu.ParentId) != 0 {
		orm.Eloquent.Table("dhp_role").Where("menu_id = ?", menu.ParentId).First(parentMenu)
		if parentMenu.Paths == "" {
	...
	orm.Eloquent.Table("dhp_role").Where("menu_id = ?", menu.MenuId).Update("paths", menu.Paths)
	return
}
