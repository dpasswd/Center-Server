...
var (
	...
	StartCmd  = &cobra.Command{
		...
		Example: "dh-passwd config -c config/settings.yml",
...
func run() {
	...
	uploadConfig, errs := json.MarshalIndent(config.UploadConfig, "", "   ") //转换成JSON返回的是byte[]
	if errs != nil {
		fmt.Println(errs.Error())
	}
	fmt.Println("logger:", string(uploadConfig))
